...
use crate::{
    boids::{ClosenessThreshold, SeparationDistance, CentreOfFlockValue, CollisionBounds, Pos, Vel, Vec2, make_a_boid, make_an_asset_entity}
};
use nalgebra::{Point2};
use rand::prelude::*;
...
impl SimpleState for MainScene {
    fn on_start(&mut self, data: StateData<'_, GameData<'_, '_>>) {
        data.world.add_resource(ClosenessThreshold(0.25));
        data.world.add_resource(SeparationDistance(0.24));
        data.world.add_resource(CentreOfFlockValue(Point2::origin()));
        data.world.add_resource(CollisionBounds::new_rect(8.0, 8.0, 0.0));
